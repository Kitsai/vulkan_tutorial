...
class HelloTriangleApplication {
  private:
    ...
    BufferWithAllocation vertexBuffer;
    BufferWithAllocation indexBuffer;

    std::vector<BufferWithAllocation> uniformBuffers;
    ...
    ImageWithAllocation depth;

    VkSampleCountFlagBits msaaSamples = VK_SAMPLE_COUNT_1_BIT;

    ImageWithAllocation color;
  ...
  private:
    ...
    int rateDeviceSuitability(VkPhysicalDevice device) {
        ...
        const bool invalid = !deviceFeatures.samplerAnisotropy || !deviceFeatures.geometryShader ||
    ...
    void createLogicalDevice() {
        ...
        if(vkCreateDevice(physicalDevice, &createInfo, nullptr, &device) != VK_SUCCESS)
            throw std::runtime_error("failed to create logical device1");
    ...
    void createFramebuffers() {
        ...
        for(size_t i = 0; i < swapChainImageViews.size(); i++) {
            std::array<VkImageView, 3> attachments = {color.view, depth.view, swapChainImageViews[i]};
    ...
    void createDepthResources() {
        ...
        createImage(swapChainExtent.width, swapChainExtent.height, 1, msaaSamples, depthFormat, VK_IMAGE_TILING_OPTIMAL,
                    VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT, VMA_MEMORY_USAGE_CPU_TO_GPU, VK_IMAGE_ASPECT_DEPTH_BIT,
                    depth);

        transitionImageLayout(depth.image, depthFormat, VK_IMAGE_LAYOUT_UNDEFINED,
    ...
    void createTextureImage() {
        ...
        createImage(texWidth, texHeight, mipLevels, VK_SAMPLE_COUNT_1_BIT, VK_FORMAT_R8G8B8A8_SRGB,
                    ...
                    VMA_MEMORY_USAGE_CPU_TO_GPU, VK_IMAGE_ASPECT_COLOR_BIT, texture);

        transitionImageLayout(texture.image, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_LAYOUT_UNDEFINED,
                              VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, mipLevels);

        copyBufferToImage(stagingBuffer.buffer, texture.image, static_cast<uint32_t>(texWidth),
        ...
        */
        ...
        generateMipmaps(texture.image, VK_FORMAT_R8G8B8A8_SRGB, static_cast<int32_t>(texWidth),
    ...
    void createColorResources() {
        ...
        createImage(swapChainExtent.width, swapChainExtent.height, 1, msaaSamples, colorFormat, VK_IMAGE_TILING_OPTIMAL,
                    ...
                    VMA_MEMORY_USAGE_CPU_TO_GPU, VK_IMAGE_ASPECT_COLOR_BIT, color);
    }
    ...
    void createUniformBuffers() {
        ...
        for(size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
            createBuffer(bufferSize, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VMA_MEMORY_USAGE_CPU_ONLY, uniformBuffers[i]);
        }
    }
    ...
    void updateUniformBuffer(uint32_t currentImage) {
        ...
        memcpy(uniformBuffers[currentImage].ptr, &ubo, sizeof(ubo));
    }
    ...
    void createImage(uint32_t width, uint32_t height, uint32_t mipLevels, VkSampleCountFlagBits numSamples,
                     VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VmaMemoryUsage memory,
                     VkImageAspectFlags aspectFlags, ImageWithAllocation& image) {
        ...
        image.view = createImageView(image.image, format, aspectFlags, mipLevels);
    }
    ...
    void destroyImage(ImageWithAllocation image) {
